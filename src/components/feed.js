...
export default function FeedList() {
  ...
  const dataFeed = [
    ...
    },
    {
      ...
      description:
        'Penyeselan pasti datang di akhir, kalau di awal namanya pendaftaran.',
      createdAt: new Date().getMinutes(),
      liked: Math.random().toString().slice(2, 3),
      comment: Math.random().toString().length,
    },
            ...
            <Stack p="4" space={3}>
              <Stack space={2}>
                ...
                <Text
                  ...
                  ml="-0.5"
                  mt="-1">
                  {item.name} <Text fontWeight="400">{item.description}</Text>
                </Text>
              </Stack>
  ...
  return (
    <FlatList
      data={dataFeed}
      renderItem={({item}) => <View>{RenderData(item)}</View>}
