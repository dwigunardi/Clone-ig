import React, { useState } from 'react';
import { View, StyleSheet, TouchableOpacity, Pressable, Alert, FlatList, StatusBar, Image } from 'react-native';
import {
    NativeBaseProvider, Box,
    Container, Text, Avatar,
    Heading, Center, HStack, VStack, Flex, ScrollView, Stack, AspectRatio, Spacer,
} from "native-base";
import LinearGradient from 'react-native-linear-gradient';

export default function FeedList() {
    const gradientColor = [
        '#FCAF45',
        '#FFDC80',
        '#E1306C',
        '#FD1D1D',
        '#F56040',
        '#F77737',
        '#833AB4',
        '#C13584',
    ]
    return (
        <ScrollView>
            <Container marginX={4}>
                <Box alignItems="center">
                    <Box w="sm" rounded="lg" overflow="hidden" borderColor="coolGray.200" borderWidth="1" _dark={{
                        borderColor: "coolGray.600",
                        backgroundColor: "gray.700"
                    }} _web={{
                        shadow: 2,
                        borderWidth: 0
                    }} _light={{
                        backgroundColor: "gray.50"
                    }}>
                        <Box>
                            {/* <HStack direction={'row'} w={'full'} >
                                <VStack>
                                    <View style={{ width: 45, height: 40, marginBottom: 5, marginTop: 2, marginHorizontal: 5, }}>
                                        <LinearGradient
                                            colors={gradientColor}
                                            start={{ x: 0.0, y: 1.0 }}
                                            end={{ x: 1.0, y: 1.0 }}
                                            style={{
                                                borderRadius: 100,
                                                padding: 2,
                                                overflow: 'hidden',
                                            }}>
                                            <Image
                                                style={{ width: 40, height: 40, borderRadius: 50, shadowColor: "#000", shadowRadius: 4, shadowOpacity: 0.5 }}
                                                source={require('../../assets/image/profile1.jpg')} />
                                        </LinearGradient>
                                    </View>
                                </VStack>
                                <VStack alignSelf={'center'}>
                                    <Text >Jhony the blues</Text>
                                </VStack>
                                <Stack flexDirection={"column"} alignSelf="center" justifyContent={'flex-end'} reversed>
                                    <Text marginLeft={"40"}>Ini end</Text>
                                </Stack>
                            </HStack> */}
                            <Flex direction='row'>
                                <Center>
                                    <View style={{ width: 44, height: 43, marginBottom: 5, marginTop: 2, marginHorizontal: 5, }}>
                                        <LinearGradient
                                            colors={gradientColor}
                                            start={{ x: 0.0, y: 1.0 }}
                                            end={{ x: 1.0, y: 1.0 }}
                                            style={{
                                                borderRadius: 100,
                                                padding: 2,
                                                overflow: 'hidden',
                                            }}>
                                            <Image
                                                style={{ width: 40, height: 40, borderRadius: 50, shadowColor: "#000", shadowRadius: 4, shadowOpacity: 0.5 }}
                                                source={require('../../assets/image/profile1.jpg')} />
                                        </LinearGradient>
                                    </View>
                                </Center>
                                <Center>
                                    <Text >Nabil Lanten</Text>
                                </Center>
                                <Spacer />
                                <Center marginRight={2}>
                                    <Text>...</Text>
                                </Center>
                            </Flex>
                            <AspectRatio w="full" ratio={16 / 9}>
                                <Image source={{
                                    uri: "https://www.holidify.com/images/cmsuploads/compressed/Bangalore_citycover_20190613234056.jpg"
                                }} alt="image" />
                            </AspectRatio>
                            <Center bg="violet.500" _dark={{
                                bg: "violet.400"
                            }} _text={{
                                color: "warmGray.50",
                                fontWeight: "700",
                                fontSize: "xs"
                            }} position="absolute" bottom="0" px="3" py="1.5">
                                PHOTOS
                            </Center>
                        </Box>
                        <Stack p="4" space={3}>
                            <Stack space={2}>
                                <Heading size="md" ml="-1">
                                    The Garden City
                                </Heading>
                                <Text fontSize="xs" _light={{
                                    color: "violet.500"
                                }} _dark={{
                                    color: "violet.400"
                                }} fontWeight="500" ml="-0.5" mt="-1">
                                    The Silicon Valley of India.
                                </Text>
                            </Stack>
                            <Text fontWeight="400">
                                Bengaluru (also called Bangalore) is the center of India's high-tech
                                industry. The city is also known for its parks and nightlife.
                            </Text>
                            <HStack alignItems="center" space={4} justifyContent="space-between">
                                <HStack alignItems="center">
                                    <Text color="coolGray.600" _dark={{
                                        color: "warmGray.200"
                                    }} fontWeight="400">
                                        6 mins ago
                                    </Text>
                                </HStack>
                            </HStack>
                        </Stack>
                    </Box>
                </Box>
                <Box alignItems="center">
                    <Box w="full" rounded="lg" overflow="hidden" borderColor="coolGray.200" borderWidth="1" _dark={{
                        borderColor: "coolGray.600",
                        backgroundColor: "gray.700"
                    }} _web={{
                        shadow: 2,
                        borderWidth: 0
                    }} _light={{
                        backgroundColor: "gray.50"
                    }}>
                        <Box>
                            <AspectRatio w="full" ratio={16 / 9}>
                                <Image source={{
                                    uri: "https://www.holidify.com/images/cmsuploads/compressed/Bangalore_citycover_20190613234056.jpg"
                                }} alt="image" />
                            </AspectRatio>
                            <Center bg="violet.500" _dark={{
                                bg: "violet.400"
                            }} _text={{
                                color: "warmGray.50",
                                fontWeight: "700",
                                fontSize: "xs"
                            }} position="absolute" bottom="0" px="3" py="1.5">
                                PHOTOS
                            </Center>
                        </Box>
                        <Stack p="4" space={3}>
                            <Stack space={2}>
                                <Heading size="md" ml="-1">
                                    The Garden City
                                </Heading>
                                <Text fontSize="xs" _light={{
                                    color: "violet.500"
                                }} _dark={{
                                    color: "violet.400"
                                }} fontWeight="500" ml="-0.5" mt="-1">
                                    The Silicon Valley of India.
                                </Text>
                            </Stack>
                            <Text fontWeight="400">
                                Bengaluru (also called Bangalore) is the center of India's high-tech
                                industry. The city is also known for its parks and nightlife.
                            </Text>
                            <HStack alignItems="center" space={4} justifyContent="space-between">
                                <HStack alignItems="center">
                                    <Text color="coolGray.600" _dark={{
                                        color: "warmGray.200"
                                    }} fontWeight="400">
                                        6 mins ago
                                    </Text>
                                </HStack>
                            </HStack>
                        </Stack>
                    </Box>
                </Box>
            </Container>

        </ScrollView >
    )
}